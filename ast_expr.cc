...
//Fix point
	int x = 0;
  left->CheckStatements();
  right->CheckStatements();

  if (left->GetTypeName() && right->GetTypeName()){
	//std::cout<<"1";
	Decl *ldecl = Program::sym_table->Lookup(left->GetTypeName());
	Decl *rdecl = Program::sym_table->Lookup(right->GetTypeName());

	if (ldecl && rdecl){
	//std::cout<<"2";
		if (!strcmp(left->GetTypeName(), right->GetTypeName())){
	//std::cout<<"3";
			return;
		}else if (typeid(*rdecl) == typeid(ClassDecl)){
	//std::cout<<"4";
	...
	//std::cout<<"5";
        ...
        }else if (ldecl && !strcmp(right->GetTypeName(), "null")){ // object = null
	//std::cout<<"6";
		return;
	}else if (!strcmp(left->GetTypeName(), right->GetTypeName())){ // non-objects
	//std::cout<<"7";
		if(strlen(right->GetTypeName()) != strlen(left->GetTypeName())){
			ReportError::IncompatibleOperands(this->op, new Type(left->GetTypeName()), new Type(right->GetTypeName()));
			x = 1;
		}
		return;
	}
